...
@Composable
fun StudioNetworkSection(
    mediaType: MediaType,
    selectedStudio: DiscoveryStudioHub?,
    selectedTvNetwork: DiscoveryTvNetworkHub?,
    onStudioSelected: (DiscoveryStudioHub?) -> Unit,
    onTvNetworkSelected: (DiscoveryTvNetworkHub?) -> Unit,
    modifier: Modifier = Modifier
) {
    Column(modifier = modifier.fillMaxWidth()) {
        if (mediaType == MediaType.Movie) {
            FilterSectionHeader(
                title = stringResource(R.string.filter_section_studios),
            ...
            FlowRow(
                ...
                val isAllSelected = selectedStudio == null
                Surface(
                    onClick = { onStudioSelected(null) },
                    ...
                    Text(
                        text = stringResource(R.string.all_studios),
            ...
            FilterSectionHeader(
                title = stringResource(R.string.filter_section_networks),
            ...
            FlowRow(
                ...
                val isAllSelected = selectedTvNetwork == null
                Surface(
                    onClick = { onTvNetworkSelected(null) },
                    shape = RoundedCornerShape(16.dp),
                    color = if (isAllSelected) MaterialTheme.colorScheme.tertiary else MaterialTheme.colorScheme.surfaceVariant.copy(
                        alpha = 0.5f
                    ),
                    ...
                    Text(
                        text = stringResource(R.string.all_networks),
                        style = MaterialTheme.typography.labelMedium,
                        fontWeight = if (isAllSelected) FontWeight.Bold else FontWeight.Medium,
                        color = if (isAllSelected) MaterialTheme.colorScheme.onTertiary else MaterialTheme.colorScheme.onSurface,
                        modifier = Modifier.padding(
                ...
                DiscoveryTvNetworkHub.entries.forEach { network ->
                    val isSelected = network == selectedTvNetwork
                    Surface(
                        onClick = { onTvNetworkSelected(network) },
                        shape = RoundedCornerShape(16.dp),
                        color = if (isSelected) MaterialTheme.colorScheme.tertiary else MaterialTheme.colorScheme.surfaceVariant.copy(
                            alpha = 0.5f
                        ),
