...
fun rememberNavigationState(
    ...
    // Map of result key to result value. Key is typically a contentKey.
    val results = rememberSaveable { mutableStateOf(emptyMap<String, Any>()) }
...
class NavigationState(
    ...
    private val resultsState: MutableState<Map<String, Any>>
) {
    ...
    fun setResult(key: String, value: Any) {
        resultsState.value += (key to value)
    }

    fun consumeResult(key: String): Any? {
        val result = resultsState.value[key]
        if (result != null) {
...
fun NavigationState.toEntries(
    ...
    return stacksInUse.flatMap { stackKey ->
        val stack = backStacks.getValue(stackKey)
        key(stackKey) {
            rememberDecoratedNavEntries(
                ...
                entryProvider = { key ->
                    ...
                    // Tag the entry with its parent tab key for transition detection
                    NavEntry(
                        key = key,
                        contentKey = entry.contentKey,
                        metadata = entry.metadata + (Nav3MetadataKeys.TabKey.toString() to stackKey),
