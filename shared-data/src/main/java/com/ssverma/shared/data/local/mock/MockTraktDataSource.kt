package com.ssverma.shared.data.local.mock
...
@Singleton
class MockTraktDataSource @Inject constructor() {
    ...
    fun getUpNextQueue(): List<TraktUpNextEpisode> {
        return mockShows.values
    ...
    fun onEpisodeWatchedToggled(showTmdbId: Int, isCurrentlyWatched: Boolean) {
        val show = mockShows[showTmdbId] ?: return
    ...
    fun onSeasonWatchedToggled(showTmdbId: Int, episodeNumbers: List<Int>) {
        val show = mockShows[showTmdbId] ?: return
    ...
    fun getMockWatchlist(): List<WatchlistEntity> {
        return listOf(
            WatchlistEntity(
                mediaId = 93405,
                mediaType = "Tv",
    ...
    fun getMockHistory(): List<WatchHistoryEntity> {
        return listOf(
            WatchHistoryEntity(
                mediaId = 550,
                mediaType = "Movie",
    ...
    fun getEpisodeTitle(showTmdbId: Int, seasonNumber: Int, episodeNumber: Int): String? {
        return mockShows[showTmdbId]?.episodeTitles?.get(Pair(seasonNumber, episodeNumber))
    }
}
